...
class PCPretrainModel(BaseModel):
    ...
    def forward_grasp(self, batch, compute_loss=False):
        ...
        if compute_loss and ('grasp_valid_masks' in batch):
            losses['grasp_bce'] = F.binary_cross_entropy_with_logits(
                pred_logits, batch['grasp_valid_masks'].float(), reduction='mean'
            )

            valid = (batch['grasp_valid_masks'] == 1)

            if ('grasp_offsets' in batch) and valid.any():
                losses['grasp_offsets'] = F.mse_loss(
                    pred_offsets, batch['grasp_offsets'], reduction='none'
                )[valid].mean()
            else:
                losses['grasp_offsets'] = pred_offsets.new_tensor(0.0)

            if ('grasp_rotations' in batch) and valid.any():
                losses['grasp_rotations'] = F.mse_loss(
                    pred_rot_matrices, batch['grasp_rotations'], reduction='none'
                )[valid].mean()
            else:
                losses['grasp_rotations'] = pred_rot_matrices.new_tensor(0.0)

            losses['total'] = losses['grasp_bce'] + losses['grasp_offsets'] + losses['grasp_rotations']
        ######################
